...
Data_fix=Data;
 fix_the_model=1000;
    ...
    Data_fix=[buffer;Data(1:1000,:);Data(1001:end,:)];
    fix_the_model=1000+size(buffer,1);
end
...
type_feature_weighting=5;
buffer=[];
[Weight,Center,Spread,rule,y,error1,population,time,classification_rate_testing,feature_weights,population_class_cluster,focalpoints,sigmapoints]=pclass_axis_parallel_updated(Data,fix_the_model,paramet,demo,ninput,mode,drift,type_feature_weighting,network(index1));
[v,vv]=size(Center);
network_parameters=v*subset+(subset)*v+(subset+1)*v*noutput;
