...
chunk_size1=round(nDatatest/nFolds);
...
for i=1:chunk_size1:nDatatest
    ...
    for j=1:size(Data1,1)
        for j1=1:size(Data1,2)
    Data2(j,j1,l)=Data1(j,j1);
        end
    end
end
buffer=[];
     ...
     if ensemblepruning1==1     && k1==size(Data,1) 
      ...
      network(m).ensemble_weight=network(m).ensemble_weight/a;
        ...
        expectationSJ(k3,k2)=(inputvariance(:,k3)+(stream(k3)-network(m).Center(k2,k3)).^(2));
            ...
            for k4=1:ninput     
            a=xek'*pik(:,:,k2);
            phi(k4,k2)=max(a)*exp((varsj1/network(m).Spread(k2,k4)^(2))-(expectationSJ1/network(m).Spread(k2,k4)));
            if (varsj1/network(m).Spread(k2,k4)^(2))>(expectationSJ1/network(m).Spread(k2,k4))
            phi(k4,k2)=0.01;
            end
            zeta(k4,k2)=phi(k4,k2)/network(m).Spread(k2,k4)^(2);
            if zeta(k4,k2)>1
                            zeta(k4,k2)=0.01;
            end
            widthTrans(k4,k2)= (inputvariance(:,k4)+(inputexpectation(:,k4)-network(m).Center(k2,k4)).^(2))/network(m).Spread(k2,k4);
            end
...
[Rselected,index1]=max(Rselected);
  fix_the_model=100;
...
buffer=[];
Data_fix=[Data;Data2(:,:,counter)];
[Weight,Center,Spread,rule,y,error1,population,time,classification_rate_testing,feature_weights,population_class_cluster,focalpoints,sigmapoints]=pclass_axis_parallel_updated(Data_fix,fix_the_model,paramet,demo,ninput,mode,drift,type_feature_weighting,network(index1));
